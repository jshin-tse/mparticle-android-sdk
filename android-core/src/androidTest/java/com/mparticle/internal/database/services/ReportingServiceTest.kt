...
class ReportingServiceTest : BaseMPServiceTest() {
    ...
    fun testInsertReportingMessage() {
        val reportingMessages: List<JsonReportingMessage> = getNReportingMessages(20, "123")
        ...
        Assert.assertEquals(
            ReportingService.getReportingMessagesForUpload(BaseMPServiceTest.Companion.database).size.toLong(),
            20
        )
    }
    ...
    fun testDeleteReportingMessages() {
        ...
        var messagesFor2 = ReportingService.getReportingMessagesForUpload(
            BaseMPServiceTest.Companion.database,
            true,
            2L
        )
        var messagesFor3 = ReportingService.getReportingMessagesForUpload(
            BaseMPServiceTest.Companion.database,
            true,
            3L
        )
        ...
        ReportingService.deleteReportingMessage(
            BaseMPServiceTest.Companion.database,
            messagesFor2[0].reportingMessageId
        )
        messagesFor2 = ReportingService.getReportingMessagesForUpload(
            BaseMPServiceTest.Companion.database,
            true,
            2L
        )
        messagesFor3 = ReportingService.getReportingMessagesForUpload(
            BaseMPServiceTest.Companion.database,
            true,
            3L
        )
        ...
        ReportingService.deleteReportingMessage(
            BaseMPServiceTest.Companion.database,
            messagesFor3[0].reportingMessageId
        )
        messagesFor2 = ReportingService.getReportingMessagesForUpload(
            BaseMPServiceTest.Companion.database,
            true,
    ...
    fun testEntryIntegrity() {
        val jsonReportingMessages: List<JsonReportingMessage> = getNReportingMessages(2)
        ...
        val reportingMessages = ReportingService.getReportingMessagesForUpload(
            BaseMPServiceTest.Companion.database,
            true,
            1L
        )
        Collections.sort(reportingMessages, java.util.Comparator { o1, o2 ->
            try {
        ...
        Collections.sort(jsonReportingMessages, java.util.Comparator<Any> { o1, o2 ->
            try {
                return@Comparator (o1.toJson().getInt("a random Number") as Int).compareTo(
                    o2.toJson().getInt("a random Number")
                )
            } catch (e: JSONException) {
                e.printStackTrace()
            }
    ...
    private fun getNReportingMessages(n: Int, sessionId: String?): List<JsonReportingMessage> {
        val reportingMessages: MutableList<JsonReportingMessage> = ArrayList<JsonReportingMessage>()
        for (i in 0 until n) {
            reportingMessages.add(
                TestingUtils.getInstance()
                    .getRandomReportingMessage(sessionId ?: java.lang.String.valueOf(Random.Default.nextInt()))
            )
        }
    ...
    private fun equals(
        ...
        val origStirng: String = jsonReportingMessage.toJson().toString()
        return reportingMessage.sessionId == jsonReportingMessage.getSessionId() && reportingString == origStirng
    }
}