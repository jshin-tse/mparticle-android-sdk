...
class MParticleJSInterfaceITest : BaseStartedTest() {
    @Rule
    var rule: ActivityTestRule<WebViewActivity> = ActivityTestRule<WebViewActivity>(
    ...
    fun testSetUserAttribute() {
        ...
        runJavascriptTest(testJavascript, object : MParticleJSInterface() {
            @JavascriptInterface
...
mParticle.eCommerce.logPurchase(transactionAttributes, product, true, %s, %s);""",
        ...
        val called = MutableBoolean(false)
        val error: Mutable<Any?> = Mutable<Any>(null) as Any
    ...
    private fun runJavascriptTest(testJavascript: String, jsInterface: MParticleJSInterface) {
        Handler(Looper.getMainLooper()).post(
            Runnable {
                ...
                val wv: WebView = rule.getActivity().findViewById<WebView>(R.id.web_view)
                wv.setWebViewClient(object : WebViewClient() {
                    override fun shouldInterceptRequest(
                        ...
                        //local server
                        return if (request.getMethod().equals("OPTIONS", ignoreCase = true)) {
                ...
                wv.setWebChromeClient(object : WebChromeClient() {
                    override fun onConsoleMessage(
                        ...
                        Logger.warning(
                            "MParticle JS sdk", message + " -- From line "
                                    + lineNumber
                        )
                    }
                })
    ...
    private val customFlags: Map<String, Any?>
        private get() {
            ...
            val fooFlags: MutableList<Any> = ArrayList()
            fooFlags.add(50)
            fooFlags.add(true)
            fooFlags.add(-27)
            val barFlags: MutableList<String> = ArrayList()
            barFlags.add("this other")
            barFlags.add("that other")
            customFlags["foo"] = fooFlags
            customFlags["bar"] = barFlags
            customFlags["baz"] = "foobar"
            customFlags["nullval"] = null
            return customFlags
        }
            ...
            val headers: Map<String, String> = object : HashMap<String?, String?>() {
                init {
                    ...
                    put(
                        ...
                        "accept, authorization, Content-Type, x-mp-key"
                    )
                    put("Via", "1.1 vegur")
                }
            }
