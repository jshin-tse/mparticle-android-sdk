...
class ConfigManagerInstrumentedTest : BaseTest() {
    ...
    fun testConfigResponseParsing() {
        ...
        TestCase.assertEquals(
            aliasMaxWindow, MParticle.getInstance()!!
    ...
    fun cachedConfigLoadedExactlyOnce() {
        ...
        val loadedCoreLocal: AndroidUtils.Mutable<Boolean> = Mutable(false)
        val loadedKitLocal: AndroidUtils.Mutable<Boolean> = Mutable(false)
    ...
    internal inner class BothConfigsLoadedListener(vararg configTypes: ConfigType?) :
        ...
        init {
            ...
            if (configTypes == null || configTypes.size == 0) {
                configTypes = arrayOf(ConfigManager.ConfigType.CORE)
            }
    ...
    internal inner class AddConfigListener(configLoadedListener: ConfigLoadedListener) :
        Configuration<ConfigManager?> {
        var configLoadedListener: ConfigLoadedListener
