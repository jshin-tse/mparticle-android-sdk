...
class IdentityApiTest : BaseStartedTest() {
    ...
    fun testUserChangeCallbackAccuracy() {
        ...
        Server.endpoint(EndpointType.Identity_Login).setResponse({it.body.previousMpid == mStartingMpid}) {
            SuccessResponse {
                this.responseObject = IdentityResponseMessage(mpid1, isLoggedIn = true)
            }
        }
    ...
    fun testIdentityChangedListenerAdd() {
        Server
            ...
            .setResponse({it.body.previousMpid == mStartingMpid}) {
                SuccessResponse {
                    responseObject = IdentityResponseMessage(mpid1, isLoggedIn = false)
                }
            }
            .setResponse({it.body.previousMpid == mpid1}) {
                SuccessResponse {
        ...
        //test that change actually took place
        result.addSuccessListener(TaskSuccessListener { identityApiResult ->
        ...
        result.addSuccessListener(TaskSuccessListener { identityApiResult ->
            ...
            assertEquals(
                identityApiResult.user.id, MParticle.getInstance()!!
    ...
    fun testIdentityStateListenerThread() {
        ...
        Handler(backgroundThread.getLooper()).post(Runnable {
            MParticle.getInstance()!!.Identity().addIdentityStateListener { user, previousUser ->
                assertEquals(Looper.getMainLooper(), Looper.myLooper())
                assertEquals(
                    user.id, MParticle.getInstance()!!
                        .Identity().currentUser!!.id
                )
                called.value = true
                latch.countDown()
    ...
    fun testCallbacksVsCurrentUser() {
        ...
        MParticle.getInstance()!!.Identity().addIdentityStateListener { user, previousUser ->
            assertEquals(
                mpid1, MParticle.getInstance()!!.Identity().currentUser!!
                    .id
            )
    ...
    fun testGetUser() {
        ...
        AccessUtils.setUserIdentities(mpid1UserIdentites, identity.currentUser!!.id)
        ...
        AccessUtils.setUserIdentities(mpid2UserIdentites, identity.currentUser!!.id)
        ...
        AccessUtils.setUserIdentities(mpid3UserIdentities, identity.currentUser!!.id)
    ...
    fun testModifyConcurrentCalls() {
        assertEquals(
            mStartingMpid, MParticle.getInstance()!!
                .Identity().currentUser!!.id
        )
    ...
    fun testGetUsersApi() {
        ...
        for (mParticleUser in MParticle.getInstance()!!
            .Identity().users) {
    ...
    fun testNoZeroMpidUser() {
        ...
        for (user in MParticle.getInstance()!!
            .Identity().users) {
            ...
            Server
                ...
                .assertWillReceive {
                    it.body.identityChanges?.let {
                        assertEquals(1, it.size)
                        it[0].apply {
                            assertEquals("customer_id", identityType)
