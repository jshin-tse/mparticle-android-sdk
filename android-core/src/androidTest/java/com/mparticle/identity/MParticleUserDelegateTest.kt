...
class MParticleUserDelegateTest : BaseStartedTest() {
    ...
    fun testGetUserAttributesListener() {
        val attributeSingles: Map<String?, String?> = RandomUtils.getRandomAttributes(5)
        ...
        AccessUtils.awaitMessageHandler()
        val userAttributesResults: Mutable<Map<String, String>?> = Mutable(null)
        val userAttributeListResults: Mutable<Map<String, List<String>>?> =
            Mutable(null)

        //fetch on the current (non-main) thread
        mUserDelegate!!.getUserAttributes({ userAttributes, userAttributeLists, mpid ->
